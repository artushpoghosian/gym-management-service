...
class JwtServiceTest {
    ...
    @Test
    void generateServiceToken_HasFixedServiceSubject() {
        String token = jwtService.generateServiceToken();
        assertThat(jwtService.extractUsername(token)).isEqualTo("gym-management-service");
    }

    @Test
    void generateServiceToken_IsValidWhenFresh() {
        String token = jwtService.generateServiceToken();
        assertThat(jwtService.isValid(token)).isTrue();
    }
